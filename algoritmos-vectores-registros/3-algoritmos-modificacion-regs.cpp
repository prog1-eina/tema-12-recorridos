 ...
 *       datos iniciales de «T» en la que todas las personas solteras tienen un
...
void distribuir(Persona T[], const unsigned n) {
    ...
    // inf >= sup --> Las personas de «T» en las componentes de índices en
    // [0, inf-1] son todas solteras y los que están en las índices
    // [inf, n - 1] son todas casados. Por lo tanto, todos los elementos de las
    // primeras «n» componentes del vector «T» han sido distribuidos.
}
 ...
 *       ordenadas de forma que tienen valores de DNI crecientes.
 */
void ordenarPorDNI(Persona T[], const unsigned n) {
    if (n != 0) {
        // Ordenación de un vector por el método de selección
        for (unsigned i = 0; i < n - 1; i++) {
            /* Las personas de las primeras i-1 componentes de «T» son las de
             * menor valor de DNI y ya están ordenadas */
            ...
            // T[iMenor] es el de menor DNI de T[i..n-1].
            //Permuta T[i] y T[iMenor]:
            permutar(T[i], T[iMenor]);
            /* Las personas de las primeras i-1 componentes del vector «T» son
             * las de menor valor de DNI y ya están ordenadas */
        }
    }
}
...
void ordenarPorEdad(Persona T[], const unsigned n) {
    if (n != 0) {
        // Se ha programado un algoritmo de ordenación de un vector por el
        // método de selección.

        // En cada iteración se permuta el elemento T[i] con el menor de los
        // elementos de T[i..n-1].
        for (unsigned i = 0; i < n - 1; ++i) {
            // Las personas de T[0..i-1] son las de más edad y ya están
            // ordenadas según edades decrecientes.
